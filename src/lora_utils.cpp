    ...
    void setFlag(void) {
        ...
        void applyRxSensitivityPatch() {
            if (radio.getMod()->SPIsetRegValue(0x8B5, 0x01, 0, 0) == RADIOLIB_ERR_NONE) {
                logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "LoRa", "Applied SX126x register 0x8B5 patch for RX improvement");
            } else {
                logger.log(logging::LoggerLevel::LOGGER_LEVEL_ERROR, "LoRa", "Failed to apply SX126x register 0x8B5 patch for RX improvement");
            }
        }
        ...
        void resetAGC() {
            ...
            while (radio.getMod()->hal->digitalRead(radio.getMod()->getGpio())) {
                if (millis() - calibrationStart > 50) {
                    logger.log(logging::LoggerLevel::LOGGER_LEVEL_WARN, "LoRa", "SX126x AGC reset: calibration did not complete within 50ms");
                    break;
                }
