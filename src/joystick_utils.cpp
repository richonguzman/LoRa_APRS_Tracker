...
typedef void (*DirectionFunc)();
    ...
    namespace JOYSTICK_Utils {
        ...
        bool checkMenuDisplayToExitInterrupt(int menu) {
            if ((menu >= 1 && menu <= 6) || menu == 10 || menu == 14 || menu == 120  || (menu >= 130 && menu <= 133) || menu == 200 || menu == 210 || menu == 1300 || menu == 1310 || (menu >= 2210 && menu <= 2212) || menu == 51 || (menu >= 50100 && menu <= 50101) || (menu >= 50110 && menu <= 50111) || menu == 9001) {
        ...
        void loop() {   // for running process with SPIFFS outside interrupt
            if (checkMenuDisplayToExitInterrupt(menuDisplay) && exitJoystickInterrupt) {
                exitJoystickInterrupt = false;
                BUTTON_Utils::longPress1();
            }
        }
